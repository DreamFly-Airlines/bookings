...
public class PaymentConfirmedIntegrationEventHandler(
    ICommandSender commandSender) : IIntegrationEventHandler<PaymentConfirmedIntegrationEvent>
{
    public async Task HandleAsync(PaymentConfirmedIntegrationEvent integrationEvent,
        CancellationToken cancellationToken = default)
    {
        try
        {
            var bookRef = BookRef.FromString(integrationEvent.BookRef);
            var command = new MarkBookingAsPaidCommand(bookRef);
